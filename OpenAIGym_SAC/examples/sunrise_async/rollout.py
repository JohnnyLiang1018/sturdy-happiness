...
def ensemble_ucb_rollout(
    ...
    masks = [] # mask for bootstrapping
    # o = env.reset()
    o_sim = client.state_init[0] ##
    o_real = client.state_init[1] ##
    for en_index in range(num_ensemble):
        agent[en_index].reset()
    next_o = None
    path_length = 0
    ...
    while path_length < max_path_length:
        ...
        for en_index in range(num_ensemble):
            ...
            _a_sim,  agent_info_sim = agent[en_index].get_action(o_sim) 
            _a_real, agent_info_real = agent[en_index].get_action(o_real) 
            ucb_score_sim = get_ucb_std(o_sim, _a_sim, inference_type, critic1, critic2,
                                    feedback_type, en_index, num_ensemble)
            ucb_score_real = get_ucb_std(o_real, _a_real, inference_type, critic1, critic2,
                                    feedback_type, en_index, num_ensemble)
        ...
        next_o, r, d ,env_info = client.request([a_max_sim,a_max_real])
        if noise_flag == 1:
            r += np.random.normal(0,1,1)[0]
        observations.append([o_sim,o_real])
        rewards.append(r)
        terminals.append(d)
        actions.append([a_max_sim,a_max_real])
        agent_infos.append([agent_info_max_sim,agent_info_max_real])
        env_infos.append(env_info)
        mask = torch.bernoulli(torch.Tensor([ber_mean]*num_ensemble))
        if mask.sum() == 0:
            rand_index = np.random.randint(num_ensemble, size=1)
